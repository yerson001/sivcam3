...
class CameraThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            if detect_classes:
                results = model.predict(source=frame_with_motion, classes=detect_classes, verbose=False)
                # Guardamos las clases detectadas en este frame
                if results[0].boxes.cls.numel() > 0:
                    current_detections = set(results[0].boxes.cls.cpu().numpy().astype(int))

                # Dibujar bounding boxes solo si la opción está activada
                processed_frame = results[0].plot() if self.show_bbox else frame_with_motion
            else:
